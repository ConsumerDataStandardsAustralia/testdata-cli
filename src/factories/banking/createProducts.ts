import {
  BankingProductBundle, BankingProductConstraintV2, BankingProductDetailV7, BankingProductEligibility,
  BankingProductDetailV5, BankingProductDetailV6, BankingProductEligibilityV2, BankingProductConstraintV3
} from 'consumer-data-standards/banking';

import { HolderWrapper } from '../../schema/cdr-test-data-schema';
...
import {
  generateDepositRateArray, generateDepositRateArrayV2, generateLendingRateArrayV2, generateLendingRateArrayV3, generateBankingProductFeaturesV2, generateBankingProductFeaturesV3,
  generateBankingProductFeaturesV4, generateBankingProductFeeArrayV2, generateBankingProductFeeArray
} from './utils';
...
export class CreateProducts extends Factory {
  ...
  private productVersion: number | null;
  ...
  constructor(options: FactoryOptions) {
    ...
    this.productVersion = options?.options?.version ? options?.options?.version as number : 7;
  }
...
This factory will accept the following options
  ...
  - productVersion:   The version of  the BankingProductDetail schema. Default is current version, ie 7  
...
Key values randomly allocated:
  ...
  private generateBankProduct(): BankingProductDetailV7 | BankingProductDetailV6 | BankingProductDetailV5 {
    let product: any = {};
    if (this.productVersion == 5) {
      product = this.generateBankingProductV5() as BankingProductDetailV5
    }
    if (this.productVersion == 6) {
      product = this.generateBankingProductV6() as BankingProductDetailV6
    }
    if (this.productVersion == 7) {
      product = this.generateBankingProductV7() as BankingProductDetailV7
    }
    return product;
  ...
  public generateBankProducts(): any[] | undefined {
    ...
    let ret: BankingProductDetailV7[] = [];
    for (let i = 0; i < count; i++) {
      const el = this.generateBankProduct() as BankingProductDetailV7;
      if (el) ret.push(el);
    }
    return ret;
  }

  private generateBankingProductV7(): BankingProductDetailV7 {
    ...
    let product: BankingProductDetailV7 = {
      ...
      product.cardArt = [
        {
          ...
          cardType: RandomBanking.BankingProductCardType(),
          cardScheme: RandomBanking.BankingProductCardScheme()
        }
      ]
    }
    ...
    if (Math.random() > 0.5) {
      product.features = generateBankingProductFeaturesV4(brandBaseUri);
    }

    // populate BankignProductDetailV4.constraints
    if (Math.random() > 0.5) {
      let constraints: BankingProductConstraintV3[] = [];
      let constraint: BankingProductConstraintV3 = {
    ...
    if (Math.random() > 0.5) {
      let eligibilities: BankingProductEligibilityV2[] = [];
      let eligibility: BankingProductEligibilityV2 = {
    ...
    if (Math.random() > 0.5) {
      ...
      product.fees = generateBankingProductFeeArrayV2(brandBaseUri);
    }

    // populate BankignProductDetailV4.depositRates
    if (Math.random() > 0.5) {
      product.depositRates = generateDepositRateArrayV2(brandBaseUri);
    }

    // populate BankignProductDetailV4.lendingRates
    if (Math.random() > 0.5) {

      product.lendingRates = generateLendingRateArrayV3(brandBaseUri);
    }
  ...
  private generateBankingProductV6(): BankingProductDetailV6 {
    let categoryLowerCase = this.category.replace(/['_']/g, '-').toLowerCase();
    let brandInfo = RandomBanking.SelectBaseBrandInfo();
    let productInfo = RandomBanking.SelectBaseBankingProductInfo(categoryLowerCase)
    let brandBaseUri = this.createBaseUriForBrand(brandInfo.brand);
    ...
    // create product with mandatory fields
    // populate the base product properties, ie BankingProductV4
    let product: BankingProductDetailV6 = {
      brand: brandInfo.brand,
      description: productInfo.description,
      isTailored: Helper.randomBoolean(0.3),
      lastUpdated: lastUpdatedTime,
    ...
    // populate BankignProductDetailV5.features
    if (Math.random() > 0.5) {
      product.features = generateBankingProductFeaturesV3(brandBaseUri);
    }
  ...
  private generateBankingProductV5(): BankingProductDetailV5 {
    let categoryLowerCase = this.category.replace(/['_']/g, '-').toLowerCase();
    let brandInfo = RandomBanking.SelectBaseBrandInfo();
    let productInfo = RandomBanking.SelectBaseBankingProductInfo(categoryLowerCase)
    let brandBaseUri = this.createBaseUriForBrand(brandInfo.brand);
    ...
    // populate BankignProductDetailV5.features
    if (Math.random() > 0.5) {
      product.features = generateBankingProductFeaturesV2(brandBaseUri);
    }
    ...
    if (Math.random() > 0.5) {
      ...
      product.lendingRates = generateLendingRateArrayV2(brandBaseUri);
    }
