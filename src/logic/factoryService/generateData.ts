...
function generateUnauthenticatedData(options: Options, unauthOptions: any, holder: HolderWrapper): Unauthenticated {
  ...
  if (unauthOptions.energy?.plansFactory) {
    ...
    let outDir = options.general?.individualFileOutDir;
    if (outDir != null) writePlansFile(result, outDir, holder);
  }
...
function generateDetailedCustomers(options: Options, customerOptions: any, holder: HolderWrapper): CustomerWrapper[] {
  ...
  // If we define an output directory for individual files
  let outDir = options.general?.individualFileOutDir;
  if (outDir != null) writeCustomerFile(result, outDir, holder);
  Helper.indentDec();

  return result;
}

function writeCustomerFile(customers: CustomerWrapper[], outDir: string, holder: HolderWrapper) {
  customers.forEach((cust: any) => {
      try {
        cust.holderId = holder.holderId;
        fs.mkdirSync(`${outDir}/${holder.holderId}/customers`, {recursive: true});
        let dst = path.join(`${outDir}/${holder.holderId}`,'customers',  `${cust.customerId}-customer.json`);toASCII
        fs.writeFileSync(dst,JSON.stringify(cust, null, 2));
      } catch (err) {
...
function writePlansFile(unauthenticated: Unauthenticated, outDir: string, holder: HolderWrapper) {
  unauthenticated.energy?.plans?.forEach((plan: any) => {
      try {
        plan.holderId = holder.holderId;
        fs.mkdirSync(`${outDir}/${holder.holderId}/plans`, {recursive: true})
        let dst = path.join(`${outDir}/${holder.holderId}`, 'plans',`${plan.planId}-energy-plan.json`);toASCII
        ...
        if (account && account.account) {
          ...
          if (accountOptions.transactionsFactory) {
            ...
            account.transactions = generateArrayOfItems(options, accountOptions.transactionsFactory,
              ...
              (factory) => {
                return factory.generateEnergyTransactions(account, customer.energy?.servicePoints);
              })
          } else {
...
function createFactories(iterations: number, general: OptionsGeneral | undefined, options: OptionsFactory): Factory[] {
  ...
  if ((options as OptionsSingleFactory).id) {
    ...
    let cnt = (options as OptionsWeighted).count ? (options as OptionsWeighted).count as number : 1;
    result = createWeightedFactories(cnt, general, options as OptionsWeighted);
